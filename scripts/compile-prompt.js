...
const MENACE_PLOW =
  "Nose plow: match the attached live take-8 plate ref_car_nose_plow.png (sha c6c8ec1f). The plow is one continuous flat blade spanning the entire nose on the black-tube bull bar, in front of both front tires, clean flanks, no side outrigger. On an away or rear-three-quarter heading force that same full-width blade to stay readable on the far nose — offset the camera until both front tires and the continuous plate can be seen. If the far nose is truly occluded, a soft implied far-nose edge of that same full-width blade is acceptable. A discrete left-front corner stub with an empty bull bar is not. No flank-mounted blade. Preserve on the Menace: welded steel mesh cages on the side glass and the windshield; a riveted metal door panel; oversize knobby tires on chrome slot wheels; faded matte pink with bare-metal / oxidized steel plating; horizontal louvers on the rear engine lid. Use ref_car_exterior.jpg for Beetle silhouette and build only; never carry its salt-flat sunset. Never attach ref_car_rear_plow or diag_nose_plow_front.";

const MENACE_PLOW_NEGATIVE =
  "No side-mounted plow, no flank-mounted blade, no left-flank blade, no blade on a side arm ahead of the front wheel, no side outrigger, no small black blade bracketed to the left-front corner, no corner-mounted stub, no empty nose bull bar, no blade on only one side of the bull bar, no plain tube bumper without a blade, no rear-mounted plow, no blade on the engine lid.";
...
function assemblePrompt(card) {
  ...
  if (cam === "POV_DIAGRAM") {
    if (card.card_id === "IV-026") {
      parts.push(
        "Cinematic photoreal still, same world as the rest of the game. 35mm full-frame equivalent, high overhead fifteen to twenty degrees off vertical looking down the travel lane, everything in focus enough to read lanes. Not driver-eye height, not a street-level left-rear three-quarter, not a 45-to-60-degree away-¾. Overcast Pacific Northwest daylight — soft, diffuse, low-contrast, gray-blue ambient. Wet asphalt, moss, oxidized steel, cold concrete. The only saturated color in frame is cranberry pink. Fine grain, no HDR, no glow, no lens flare. This is a photograph of real vehicles on a real street, not a map, not an infographic, not a vector diagram."
      );
    } else {
  ...
  if (card.card_id === "IV-026") {
    parts.push(
      "The Beetle is in readable mid-motion on a wet downtown block: rolling straight away toward three LARGE hazards in her SAME travel lane about two Beetle-lengths ahead of the nose blade — a cargo van with its side door open into her path, a person standing in that lane, an upright bicycle in that lane — tight, immediate, not on the sidewalk, not at a distant house, not six-to-eight lengths up the block. Wet knobby tire spray, wheels mid-roll, not parked still. Body parallel to the skip-dashed line. Camera is high-oblique, fifteen to twenty degrees off vertical, looking down the travel lane from slightly behind and above so BOTH lanes read and the FRONT bull bar with the full-width blade is visible at the far leading end. That front black-tube bull bar MUST carry one continuous full-width flat plate blade spanning BOTH front tires — match attached live take-8 plate ref_car_nose_plow.png. Do not invent a small black blade on the left-front corner. Do not flip the car toward the camera to show the plate. If the far nose is occluded, keep a soft implied far-nose edge of the same full-width blade — never a corner stub. Do not put a black tube bar across the REAR — the rear is the stock short Beetle bumper plus horizontal engine-lid louvers. Left flank is riveted plate only. Preserve mesh cages on side glass and windshield, riveted door plate, knobby tires on chrome slot wheels, faded pink + bare metal. Attach only ref_car_exterior.jpg and ref_car_nose_plow.png; do not attach ref_diagram_style.png, ref_car_rear_plow, or diag_nose_plow_front."
    );
  }
  const quietNamed =

...
const FRAMING = {
  ...
  POV_COCKPIT_ENCORE:
    "Camera is inside the cabin, over the wheel, looking forward through the windshield. A small suede-wrapped racing steering wheel across the bottom of frame. Stripped dashboard, bare metal, exposed wiring, a single aftermarket tachometer, a toggle-switch panel with one large guarded red switch clearly separate from the others. Welded roll cage tubing crossing the A-pillars. Intact windshield glass, clean and unbroken, no mesh — a wide open view. The cabin is enclosed: solid roof headliner above the glass. Do not show the roof PA horns from inside.",
  POV_COCKPIT_LEDGER:
...
function assemblePrompt(card) {
  ...
  if (encoreNamed) {
    negs.push(
      encoreCockpit
        ? "No open cabin, no missing windshield, no stripped glass, no mesh over the windshield, no bars across the glass, no window nets, no convertible, no sky where the roof should be, no PA horns inside the cabin."

# Act V — five-card split-paste (Claude follow-up)

Text only. No stills. No seed. The five patched cards from the muted-read FAIL pass, plus the V-001 cite fix.

Authority is the card JSON on PR #151. If this file and a card disagree, the card wins.

Full 13-card packet: `pack/ACT_V_13CARD_MUTED_READ.md`.

## Flags locked for this split

1. V-001 DOL is **`5.5 Focus`** (eyes-up). Not 4.12. Not 4.16.
2. **`4.10` keep-right is cut** from Act V. Thirteen is thirteen. Not on V-006. Not on V-011.
3. Workzone plate ≠ V-008. V-008 stays exit-ramp posted-25. Candid later attach: V-006. Separate PR promotes the plate.
4. **`daylight_fail` ENDS the run.** V-013 is one dusk scene. Overrun is not a V-013 variant.

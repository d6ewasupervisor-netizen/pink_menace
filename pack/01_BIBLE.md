...
- `ref_car_nose_plow.png` — **nose plow attachment.** Overcast square plate (Claude PASS take 5, 2026-09-11). One continuous wide flat nose blade spanning the FULL WIDTH of the front bull bar, outer edges roughly level with the front wheels; no narrow flap; no flank-mounted blade; nothing on the rear. The plate is toward-camera; on `away_from_camera` copy the blade only and rotate the car so that blade sits on the FAR front. Preserve mesh on side glass and windshield, riveted metal door, knobby tires on chrome wheels, faded pink + bare-metal. Attach with `pink_menace_exterior` alongside `ref_car_exterior.jpg`. `ref_car_rear_plow.jpg` and any flank / corner plow plate are out of the compile pool.

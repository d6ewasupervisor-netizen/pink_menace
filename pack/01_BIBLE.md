...
- `ref_car_sheet.png` — Menace four-view lock. Rear 3/4 is the nose plow at the far leading end — the plate the model must see on away-from-camera compiles.
...
- `ref_diagram_style.png` — optional overhead look lock. Diagram compiles attach `ref_car_sheet.png` and `ref_car_exterior.jpg` for the Menace's build. Lighting is still overcast PNW, never the salt-flat sunset.

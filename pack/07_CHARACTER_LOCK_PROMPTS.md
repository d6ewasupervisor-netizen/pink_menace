...
6. After the lock: winner goes in `refs/` and `LOCKS.md`. Unpromoted and superseded takes leave the generator pool. **Discarded** takes leave the pool entirely — they are not superseded-but-available. Plate locks travel in their own PR(s); do not bundle them with card art.
7. **Y4 Encore cockpit rerun:** do not attach Y4 take 1 (`sha256` prefix `85bd45ee`) as the interior reference — stripped bare-metal contradicts seeded Encore card frames. Do not reintroduce Y4 take 7 (floor shifter). Encore is automatic: two pedals, no clutch, no floor shifter. Left-hand drive. Attach `ref_encore_sheet.png` (PR #14 / Y3 take 12). Match seeded Encore interiors, not the rejected plate. Candidates stay in `cards/takes/` until a muted-read winner is promoted.
...
> Cinematic photoreal still. 35mm full-frame equivalent, f/2.0, shallow depth of field. Overcast Pacific Northwest daylight through the windshield — soft, diffuse, low-contrast, gray-blue. Desaturated palette; the only bright element is retroreflective silver-white. Camera inside the vehicle, over the wheel, looking forward. Left-hand drive. A small suede-wrapped racing steering wheel across the bottom of frame. Match seeded Encore card interiors: moulded dark 1990s hatchback dash and instrument binnacle (worn, taped), not a stripped open-air bare-metal tub. A single aftermarket tachometer, a toggle-switch panel with one large guarded red switch clearly separate from the others. Welded roll cage tubing crossing the A-pillars inside an enclosed cabin. Intact factory windshield glass, clean and unbroken, no mesh — wide open view. Rain beads on the outside of the glass. Beyond the glass, wet empty city street under flat gray sky. A dead wireless handheld microphone clipped to the cage tube at the right edge of frame. Automatic: two pedals only if the footwell is in frame; no clutch; no floor shifter. Fine grain, slight vignetting, no HDR, no glow, no lens flare.
>
> *[negative block]* · Aspect ratio 2:3

...
- **Both cats only in I-001 and I-003.** After the lot: Gracie alone, empty seat — except I-009, whose accepted door-mirror plate has no passenger seat in frame. Gracie stays in that card's scene copy.
...
| I-009 | take 19 | **seeded** — inside-cabin left door-mirror, empty aisle in the glass. Passenger seat and Gracie are out of frame; they stay in scene copy, not in the plate. Confirmed still `cards/I-009.png` == `cards/takes/I-009-take-19.png`. |
...
| Takes | Verdict | Notes |
|-------|---------|-------|
| 1–8 | superseded | Old locked plates. Replaced live by take 19. |
...
| 17–20 | **muted-read (Claude, 11 Sep)** | Camera inside the cabin is fixed on all four. **17 / 18 / 20 FAIL** — Gracie on the left seat in front of the wheel. **19 ACCEPTED** — left-seat eye height, left door mirror, empty aisle in the glass; no passenger seat in frame. Brief `read` and occupancy (no Gracie / no passenger seat in `image_brief`) adjusted to match the plate. Scene copy unchanged. |
| 21–24 | **not generated** | Inpaint of take 19 aborted. The passenger seat is not in those pixels; adding Gracie there is a recompose (forbidden fifth round). See `cards/I-009.inpaint.md`. Accepted take 19 instead. |

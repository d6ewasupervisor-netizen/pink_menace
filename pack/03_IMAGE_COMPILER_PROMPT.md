...
You MUST:
  ...
  - attach the ego vehicle lock to every POV_DIAGRAM for build only; no faces.
    Ali: ref_car_exterior.jpg + ref_car_rear_plow.jpg. Deac: ref_ledger_sheet.png. Never attach a
    Menace lock (ref_car_exterior.jpg, ref_car_rear_plow.jpg, ref_cockpit.jpg, Ali, Gracie) on a Deac card.
...
- POV_DIAGRAM — the same photoreal game world as every other card. Camera
  ...
  `ref_car_exterior.jpg` and `ref_car_rear_plow.jpg`; Deac diagrams get `ref_ledger_sheet.png` and never
...
| Pink Menace exterior | `ref_car_exterior.jpg` and `ref_car_rear_plow.jpg`. Front 3/4 is silhouette and build — **nose_plow: standard plate equipment**. Rear 3/4 is plow authority for behind / left-rear / `POV_DIAGRAM`: wide flat blade on the black tube bull bar at the far leading / nose end. Not scene-optional. Not left-flank / side-mounted. Never copy either plate's salt-flat sunset. |
...
| `POV_DIAGRAM` (Ali) | `ref_car_exterior.jpg` + `ref_car_rear_plow.jpg` for Beetle build and plow. Rear 3/4 plate is plow at the far leading end. Overcast PNW lighting — never the salt-flat sunset. No faces. |
| `POV_DIAGRAM` (Deac) | `ref_ledger_sheet.png` only. Never `ref_car_exterior.jpg` or `ref_car_rear_plow.jpg`. |
...
**Menace nose plow.** The wide flat plow blade on the black tube bull bar is standard `pink_menace_exterior` plate equipment — same class as Encore `pedal_count: 2`. Always at the far leading / nose end. Not scene-optional. Not left-flank / side-mounted. Reject a missing plow, a left-flank-only blade, or a thin bar without a blade. Attach `ref_car_rear_plow.jpg` with `ref_car_exterior.jpg` so the model sees the rear 3/4 (plow at the far nose). A LOCKS.md sentence without that plate will not hold.

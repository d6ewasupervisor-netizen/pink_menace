...
3. **`ref_ribbon_workzone` — PASS promote wave C take-1** (not take-2). Diagonal MUTCD-orange cone pinch across the closed lane. Wave A both FAIL (longitudinal cone line). Live: `refs/ref_ribbon_workzone.png`. Token `ribbon_workzone`. Wave-C take-2 stays runner-up candidate only.
...
Exact bytes of `refs/candidates/ribbon-deck-take-4.png`, `refs/candidates/ribbon-rig-take-1.png`, and `refs/candidates/ribbon-workzone-c-take-1.png`. Candidate closest/runner copies stay in the pool (deck closest remains take-2; Claude promoted the runner). Wave-C take-2 stays runner-up only.
...
Wave A (PR #149) closest/runner copies stay in `refs/candidates/` as `ribbon-{deck,rig,workzone}-take-N.png`. Live deck/rig/workzone are listed under Promoted. Workzone candidate copies below keep wave C take-2 as runner-up only.
...
| workzone C | **Runner-up** (not lock) | `refs/ref_ribbon_workzone_candidate_runnerup.png` | c-2 | `5750120efc39dfef2dc8347ed2eca5461cfe16cc8112467e85a98dd246521de1` |
...
Deck take-4, rig take-1, and workzone wave-C take-1 are live. Real shas in `refs/LOCKS.md`.

## Workzone regen — wave C (c-1 PROMOTED; c-2 runner-up only)

Wave A (takes 1–8 + b-1) failed the diagonal-taper hard gate. Wave C is eight new takes against the locked brief plus that gate. Claude muted-read **PASS** on c-1. Live: `refs/ref_ribbon_workzone.png`. Token `ribbon_workzone` is in REF_MAP. Wave-C take-2 stays runner-up candidate only — do not promote.
...
- c-1 is live. c-2 remains runner-up candidate only.

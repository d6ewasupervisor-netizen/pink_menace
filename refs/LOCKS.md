...
| Pink Menace cabin | take 16 | `ref_menace_cabin.png` | Claude muted-read PASS on PR #73 candidate take 16. Flat painted Type 1 dash, single nacelle, unbranded wheel, ball-knob floor shifter, coarse panel mesh. `pink_menace_interior` attaches this only. `ref_cockpit.jpg` is poisoned (tablet / digital 112.0-class) — out of the compile pool. See `CANDIDATE_MENACE_CABIN.md`. |
...
| Cat carrier shell | IV-028 take 70 crop | `ref_carrier.png` | Charcoal / near-black hard-shell only. Latch + grate. Attach on in-carrier frames alongside studio coats. Olive take-72 discarded. Do not attach on seat-cat frames (e.g. IV-018). |

...
| Diagram language | style lock | `ref_diagram_style.png` | Optional. Overhead photoreal of the Menace on a two-lane road, not a schematic. Prefer `ref_car_exterior.jpg` for build. |
...
| Ledger cockpit | D4 take 2 | `ref_ledger_cockpit.png` | **Poisoned lock — do not trust until retaken.** Take 2 mounted the clipboard on the passenger mesh and blocked the right half of the road. D4 prompt now forbids that. Retake before any Ledger cockpit recompile; attaching this file is how the clipboard keeps returning. Wiper-slot cut, thermos, high seating, round west-coast mirror are the keepers. |

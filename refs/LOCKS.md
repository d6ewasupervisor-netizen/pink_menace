...
| Pink Menace exterior | lock sheet + plow plate | `ref_car_exterior.jpg`, `ref_car_nose_plow.png` | `ref_car_exterior.jpg` = silhouette and build (salt-flat sunset is **not** game lighting). `ref_car_nose_plow.png` = Claude PASS take 5 (2026-09-11), overcast square plate. **Plow:** a full-width nose blade on the front bull bar; no flank-mounted blade; nothing on the rear. **Preserve:** welded mesh on side glass and windshield; riveted metal door panel; knobby tires on chrome wheels; faded pink + bare-metal body. Never attach `ref_car_rear_plow.jpg` / `.png` or any flank / corner plow plate — out of the compile pool. Lossless exterior copy: `ref_car_exterior.png`. |
...
**Plow:** a full-width nose blade on the front bull bar. No flank-mounted blade. Nothing on the rear.

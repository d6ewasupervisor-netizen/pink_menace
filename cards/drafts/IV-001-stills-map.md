...
## Accepted / seed-ready (Claude Batch D PASS)

These two named spots are locked. Same delivery path as III-001 (`public/game/ride/<card>/<spot>.webp`). No scored `cards/IV-001.json` — do not invent one. Live Postgres `npm run seed -- IV-001` cannot run until that JSON exists and `DATABASE_URL` is in the environment.

| Frame | Accepted take | Live named-spot plate |
|---|---|---|
| count-in | `cards/takes/IV-001-count-in-take-5.png` | `public/game/ride/IV-001/count-in.webp` |
| handoff | `cards/takes/IV-001-handoff-take-1.png` | `public/game/ride/IV-001/handoff.webp` |

Count-in-5: Yuna profile at the wheel, two fingers counting, street through rain glass. Handoff-1: Ali leaning in from the bench, hand on the empty wheel, driver seat empty.

Brake / person / tracks stay reshoot — not accepted, not encoded here.

Encore cockpit lock path (confirm only; do not regenerate): `refs/ref_encore_cockpit.png` on PR #26 (`cursor/fail-regens-glass-portrait-4392`, Y4 take 7).

## Closest take (1 of 4+)

Muted-read pick for the unseeded frames. Count-in and handoff are accepted above.

| Frame | Closest | Why |
|---|---|---|
| count-in | `IV-001-count-in-take-5.png` | **Accepted / seed-ready.** Two-beat hand + street through rain glass. |
...
| handoff | `IV-001-handoff-take-1.png` | **Accepted / seed-ready.** Empty seat is in frame; Ali reaches the empty wheel. |

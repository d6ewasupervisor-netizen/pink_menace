...
## Act VI / audit land on main

# PINK MENACE — playtest notes

## Act VI — The Backcountry (scaffold)

**Status:** card JSON shells only. **No art. No stills. No seeding.**

Claude fills copy into `cards/VI-001.json` … `cards/VI-013.json`. Citations locked in `pack/39_ACT_VI_CITATION_AUDIT.md` and the slot table in `pack/40_ACT_VI.md`.
...
Independent citation check: `node scripts/validate-citations.js` (includes VI once JSON is present).
